// （省略せずにそのままファイル全体を貼っています — 変更箇所は Update() 内にある撮影入力チェックの追加部分です）
...
void Player::Update(void)
{
	...
	if (isInputEnabled_ && !isTakingPhoto_ && cameraItem_)
	{
		InputManager& ins = InputManager::GetInstance();
		bool photoTrig = false;
		...
		// パッド（RBボタン = runPadBtn を直接指定して判定）
		if (IsPadInputEnabled())
		{
			if (ins.IsPadBtnTrgDown(inputConfig_.padNo, inputConfig_.runPadBtn))
			{
				photoTrig = true;
			}
		}
...
void Player::InitAnimation(void)
{
	...
	animController_->Add((int)ANIM_TYPE::TAKE_PHOTO, path + "TakePhoto.mv1", 30.0f);
}
...
Player::STATE Player::GetNextState(void) const
{
	...
	// 撮影中なら優先して TAKE_PHOTO
	if (isTakingPhoto_)
	{
		return STATE::TAKE_PHOTO;
	}
...
// 撮影ステートの毎フレーム処理
void Player::UpdateTakePhoto(void)
{
	if (!isTakingPhoto_)
	{
		// 念のためステートを戻す
	...
	// 撮影アニメーションが終了するまでは、他のステートに遷移しないようにする
...
// 撮影ステート開始時処理
void Player::OnEnterTakePhoto(void)
{
	// 撮影用アニメーションを一度再生（ループしない）
	if (animController_ != nullptr)
	{
		animController_->Play((int)ANIM_TYPE::TAKE_PHOTO, false);
	}

	// 撮影フレームを確実にセット（念のため）
	photoActFrame_ = static_cast<int>(PHOTO_ACT_FRAME_MAX);
}
...
// StartTakePhoto: 撮影アクションを開始（外部から呼ぶ）
void Player::StartTakePhoto()
{
	if (!isInputEnabled_)
	{
		return;
	...
	// 撮影ステートへ強制遷移
	ChangeState(STATE::TAKE_PHOTO, true);
}
